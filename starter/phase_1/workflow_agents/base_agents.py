        ...
        response = client.chat.completions.create(
            ...
            messages=[
                # TODO: 3 - Add a system prompt instructing the agent to assume the defined persona and explicitly forget previous context.
                {
                    "role": "system",
                    "content": (
                        f"You are now adopting the persona: '{self.persona}'. "
                        f"Forget any previous conversation context and respond strictly "
                        f"based on this persona."
                    )
                },
...
class KnowledgeAugmentedPromptAgent:
    def __init__(self, openai_api_key, persona, knowledge):
        ...
        self.openai_api_key = openai_api_key
        self.persona = persona
        self.knowledge = knowledge  # TODO: 1 - Create an attribute to store the agent's knowledge.
...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions):  # TODO: 2
            ...
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                # TODO: 5 - message structure for evaluation
                messages=[
                    {
                        "role": "system",
                        "content": (
                            f"You are an evaluation agent with persona: {self.persona}. "
                            f"Evaluate the answer strictly based on the given criteria."
                        )
                    },
                    {
                        "role": "user",
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    # TODO: 6 - message structure to generate correction instructions
                    messages=[
                        {
                            "role": "system",
                            "content": (
                                "You are a helpful assistant that writes clear, concise "
                                "instructions to improve incorrect answers."
                            )
                        },
                        {
                            "role": "user",
        ...
        return {
            # TODO: 7 - Return final response, evaluation, and number of iterations
